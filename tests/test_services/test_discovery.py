...
async def test_stage1_pass_matches_target_role():
    ...
    profiles = [SearchProfile(name="Test", target_roles=["Backend Engineer"], allowed_locations=[], min_score=60)]
...
async def test_stage1_pass_rejects_irrelevant_text():
    from backend.services.discovery import SearchProfile, _stage1_pass
    profiles = [SearchProfile(name="Test", target_roles=["Backend Engineer", "ML Engineer"], allowed_locations=[], min_score=60)]
...
async def test_process_job_filters_stage1_failure(session):
    ...
    profiles = [SearchProfile(name="AI", target_roles=["Backend Engineer"], allowed_locations=[], min_score=65)]
...
async def test_process_job_skips_duplicate_hash(session):
    ...
    profiles = [SearchProfile(name="AI", target_roles=["Backend"], allowed_locations=[], min_score=65)]
...
async def test_process_job_scores_relevant_job(session):
    ...
    run_row = (await session.execute(select(DiscoveryRun).where(DiscoveryRun.id == run.id))).scalar_one()
