...
import { Download, FileText, Plus, RefreshCw, Send, Sparkles, Undo2 } from "lucide-react";
...
import { PrimaryButton, SecondaryButton, StatusPill } from "../components/portal";
import type {
...
export function ResumeEditor() {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-[rgba(0,0,0,0.06)] bg-white p-5 sm:flex-row sm:items-center">
        <div>
          <p className="mb-1 flex items-center gap-1.5 font-mono text-xs uppercase tracking-[0.15em] text-[#71717a]">
            <FileText className="size-3.5" />
            {isFork ? "Tailored for this role" : "Master resume"}
          </p>
        ...
        <div className="flex items-center gap-2">
          ...
          {isFork && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-[rgba(0,0,0,0.08)] bg-white px-3 py-2 text-sm font-medium text-[#71717a] transition-colors hover:border-[rgba(0,0,0,0.15)] hover:text-[#0f0f17]"
            >
              ← Results
            </Link>
          )}
