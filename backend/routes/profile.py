...
async def get_profile_status(db: AsyncSession = Depends(get_db)) -> ProfileStatusResponse:
...
async def refresh_github(db: AsyncSession = Depends(get_db)) -> GitHubRefreshResponse:
    """Fetch all repos from GitHub, update cache, rebuild merged_profile."""
    profile, repos_updated = await refresh_github_cache(db)
