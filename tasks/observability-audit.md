...
| 8 | User feedback loops | **MISSING (feedback); evals scaffolded-but-lost** | No feedback model/table/route or frontend control (verified absent in `models.py`, `routes/`; only `generating` UI state matches the grep). An eval harness existed locally — `tests/test_evals/__pycache__/` holds bytecode (compiled 2026-05-30) for `test_consistency.py` (runs `MatchScorerAgent` 3×, asserts score variance ≤ 15) and `test_validators.py` (imports `backend.evals.validators` with `validate_job_parser/match_scorer/gap_analyst/resource_planner/cover_letter/resume_tailorer`). Source was **never committed** and the originating checkout (`~/Desktop/Job_Ready_Agent`) is gone; `README.md:96,102` references `make eval-consistency` but the Makefile has no such target. Committed heuristic scoring: `MatchScorerAgent` (LLM) + discovery gating (`_stage1_pass:85`, `_match_profiles:133`). Per-agent Pydantic validation in agents (`job_parser.py:35`, `match_scorer.py:20`). | No feedback capture, and no eval scorer consumes feedback. `backend/evals/validators.py` is reconstructable from the recovered function names but is out of scope here; the feedback track lands capture + a documented hook into it. |
...
2. **Lost evals harness.** `backend/evals/validators.py` (six `validate_*` fns) + the
   `tests/test_evals/` consistency/validator suite existed locally (bytecode dated 2026-05-30)
   but the source is unrecoverable and was never committed. Reconstructing it is a separate
   track; the feedback work only wires the hook.
